...
作成日：2026-09-19。初稿をGitHub Pagesへ公開済み（コミット`1770cf3`）。2026-09-20、提供画像に沿って名称を「籠やみらい相談」に変更し、相続前の準備から専門家連携・実行支援までの内容へ改修。詳細は`修正履歴_20260920.md`を参照。

## 内容とデザイン

- ページ：`kazokumiraikaigi.html`
- プレビュー：`http://127.0.0.1:8792/kazokumiraikaigi.html`（このPCでサーバーが稼働している間）
- 初稿の参考：[柴田行政書士事務所 家族未来会議](https://shibata-legal.com/kazokumiraikaigi/)。現在の原稿は2026-09-20のユーザー提供画像と修正指示を優先。
- 既存サイトの共通ヘッダー・フッター、Shippori Mincho／Zen Kaku Gothic、青とゴールドの色を継承。
- 文章は籠やの既存ナレッジに合わせて再構成。第三者の口コミ・人物プロフィール・メディア実績は転載していない。
- サービス名は「籠やみらい相談」。費用・期間は支援内容を確認して事前に個別案内する。
- 営業時間はユーザー指示により10:00〜18:00／水曜定休を維持。
- 不動産・FP・エータイ・弁護士・税理士・介護の6領域と、エータイ・ケアマネージャーとの協業説明を追加。
