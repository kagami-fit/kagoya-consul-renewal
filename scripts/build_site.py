...
def header(current: str = "") -> str:
    ...
    drawer_links = [
        ...
        ("properties.html", "物件情報", "PROPERTIES"),
...
<footer class="site-footer">
  <div class="footer-top">
    ...
    <div class="footer-cols">
      ...
      <div class="footer-col"><h3>Company</h3><a href="cases.html">相談事例</a><a href="team.html">専門家・チーム</a><a href="about.html">会社情報</a><a href="insights.html">知る・読みもの</a></div>
...
<section class="hero" data-hero>
  <div class="hero-inner">
    ...
    <div class="hero-visual">{img("wp-hero.jpg", "現行の株式会社籠や公式サイトに掲載されている海辺の街並み", eager=True)}<span class="hero-caption">SITUATION / VALUE / OPTIONS / RISK</span><span class="hero-line"></span></div>
  </div>
</section>
...
<section class="c-section c-section--soft">
  <div class="wrap knowledge">
    <div class="knowledge-visual reveal">{img("wp-sell.jpg", "住宅模型と計算機を使って不動産の価値を検討するイメージ")}</div>
...
<section class="c-section">
  <div class="wrap reason-grid">
    <div class="reason-image">{img("wp-consultation.jpg", "不動産について相談する家族と担当者")}</div>
...
<section class="c-section c-section--soft">
  <div class="wrap">
    <div class="service-feature">
      <div class="service-feature__image">{img("wp-invest.jpg", "住宅模型と資料を使って資産計画を検討するイメージ")}</div>
...
<section class="c-section">
  ...
  <div class="wrap proof-layout">
    ...
    <div><div class="proof-gallery"><figure>{img("wp-case-map.jpg", "現行サイトの事例記事に掲載された複数所有者の土地関係図")}<figcaption>RIGHTS MAP / 権利関係を整理</figcaption></figure><figure>{img("wp-property-street.jpg", "現行サイトに掲載されている住宅地の現地写真")}<figcaption>SITE / 現地と周辺を確認</figcaption></figure><figure>{img("wp-property-interior.jpg", "現行サイトに掲載されている明るい住戸室内")}<figcaption>USE / 利用状態を確認</figcaption></figure></div><div class="proof-story"><h3>成果の数字より、判断できる状態をつくる。</h3><p>事例の条件や結果は物件ごとに異なります。ここでお伝えしたいのは、複雑さをそのままにせず、所有者の意思・法的条件・市場性を分けて考える姿勢です。</p></div></div>
  </div>
</section>

<section class="c-section c-section--soft">
  <div class="wrap message-layout">
    <div class="message-photo">{img("wp-consultation.jpg", "現行の株式会社籠や公式サイトに掲載されている不動産相談の様子")}</div>
...
<section class="final-cta">
  <div class="final-cta__bg">{img("wp-renovation.jpg", "現行サイトに掲載されている集合住宅の外観")}</div>
...
SERVICES = page_hero("Services", "サービス", "相談内容に合わせて、必要な道筋を組み立てます。", "wp-sell.jpg", "住宅模型と計算機を使って不動産の価値を検討するイメージ") + '''
...
<section class="page-sec"><div class="wrap image-split"><div class="image-split__image">''' + img("wp-inherit.jpg", "現行サイトに掲載されている明るい空き家の室内") + '''</div><div class="image-split__body"><span class="eyebrow">Not one answer</span><h2>売ることだけが、相談の出口ではありません。</h2><p>保有、管理、賃貸、改修、売却。時間や費用を含めて比較し、納得できる方針を選ぶための土台をつくります。</p><a class="btn" href="contact.html">相談を始める</a></div></div></section>'''


DETAILS = {
    "sale-consulting.html": ("Sale consulting", "不動産売却・調査相談", "価格だけで決める前に、条件と可能性を確認します。", "wp-sell.jpg", "住宅模型と計算機を使って不動産の価値を検討するイメージ", "売る前", "査定は判断材料の一つです。接道、建築条件、権利関係、利用状態などを確認し、売却以外の可能性も含めて考えます。", [("物件の現況", "利用状態、管理状況、修繕の必要性を確認します。"), ("権利・法的条件", "登記、接道、境界、共有など、価値と実行に関わる条件を整理します。"), ("市場と活用可能性", "売却時の見え方に加え、保有・賃貸・改修の可能性を比較します。")], [("理由を説明できる", "価格や方針の根拠が分かり、家族や共有者と話しやすくなります。"), ("売り急がずに決められる", "希望時期と負担を踏まえ、進め方を選べる状態を目指します。")]),
    "inheritance-vacant-house.html": ("Inheritance & vacant house", "相続・空き家相談", "名義・共有・管理負担を整理し、次の判断へつなぎます。", "wp-inherit.jpg", "現行サイトに掲載されている明るい空き家の室内", "相続後", "相続不動産は、家族の意向、期限、維持費が同時に動きます。売却の前に、誰が何を判断するのかを一つずつ整理します。", [("所有者と共有者", "登記名義と関係者の意向を確認します。"), ("空き家の状態", "建物、設備、近隣への影響、維持費を確認します。"), ("期限と専門家", "税務・法務の確認が必要な項目を切り分けます。")], [("家族で話す材料ができる", "感情と実務を分け、共通の判断材料をつくります。"), ("売却以外も比べられる", "保有、管理、活用、売却を同じ条件で比較します。")]),
    "purchase-asset.html": ("Purchase & asset", "不動産購入・資産形成相談", "良い面だけでなく、見えにくい条件とリスクも確認します。", "wp-buy.jpg", "住宅模型を並べて購入条件を比較するイメージ", "買う前", "購入予定の不動産は、立地や価格だけでなく、収益性、修繕、融資、出口まで含めて判断する必要があります。", [("物件条件", "構造、築年、法的条件、修繕履歴を確認します。"), ("収支と資金計画", "収入、支出、空室、融資条件を整理します。"), ("将来の出口", "保有期間、売却可能性、変更しにくい条件を確認します。")], [("利点と注意点を比べられる", "購入後に気づきやすい負担も事前に整理します。"), ("生活全体で判断できる", "住まい・教育・老後など資金計画との関係も考えます。")]),
    "property-management.html": ("Management & use", "不動産管理・活用相談", "一室から一棟まで、持ち続けるための課題を整理します。", "wp-manage.jpg", "現行サイトに掲載されている管理対象の集合住宅", "持つ間", "管理は集金や連絡だけではありません。空室、修繕、入居者対応、将来の売却まで含めて、負担と価値の両方を見ます。", [("管理状態", "契約、入居、修繕、清掃、連絡体制を確認します。"), ("収支と空室", "固定費、修繕費、募集条件、稼働状況を整理します。"), ("中長期の方針", "保有、改修、売却の判断時期を考えます。")], [("優先順位が分かる", "今すぐ必要な対応と、後で検討できることを分けます。"), ("管理と出口をつなげる", "持ち続けることと将来の売却を別々に考えません。")]),
    "corporate-benefits.html": ("Corporate benefits", "法人向け不動産相談", "従業員の住まいと資産形成を支える選択肢をご提案します。", "wp-consultation.jpg", "不動産について相談する家族と担当者", "従業員支援", "住宅購入や資産形成は、従業員にとって大きな意思決定です。企業の福利厚生の一つとして、不動産と資金計画を相談できる窓口を整えます。", [("制度の目的", "対象者、利用場面、社内案内の方法を確認します。"), ("相談領域", "住まいの購入、売却、資産形成など必要な範囲を整理します。"), ("運用方法", "個別相談へのつなぎ方と、個人情報の扱いを確認します。")], [("従業員の判断を支えられる", "営業ではなく、条件比較と理解を助ける機会をつくります。"), ("社内の相談先が明確になる", "不動産の悩みを抱え込まず、専門窓口へつなげます。")]),
}


CASES = page_hero("Cases", "相談事例", "難しい案件ほど、状況を分けて考えます。", "wp-case-map.jpg", "現行サイトの事例記事に掲載された複数所有者の土地関係図") + f'''
...
<section class="page-sec alt"><div class="wrap"><div class="case-board"><figure>{img("wp-case-map.jpg", "現行サイトの事例記事に掲載された複数所有者の土地関係図")}<figcaption>01 / 権利関係と区画を可視化</figcaption></figure><figure>{img("wp-property-street.jpg", "現行サイトに掲載されている住宅地の現地写真")}<figcaption>02 / 現地と周辺状況を確認</figcaption></figure><figure>{img("wp-property-interior.jpg", "現行サイトに掲載されている明るい住戸室内")}<figcaption>03 / 利用状態と活用可能性を検討</figcaption></figure></div></div></section>
...
TEAM = page_hero("Team", "専門家・チーム", "不動産を起点に、必要な専門知識へつなぎます。", "wp-consultation.jpg", "不動産について相談する家族と担当者") + f'''
<section class="page-sec"><div class="wrap profile-grid"><div>{img("wp-consultation.jpg", "現行の株式会社籠や公式サイトに掲載されている不動産相談の様子")}</div><div><span class="eyebrow">Representative</span><h2>売却の前に、納得できる判断を支えたい。</h2><p>不動産には、一つとして同じ条件のものがありません。分からないまま決めるのではなく、状況と理由を理解し、自分で選べる状態をつくる。そのために、調査と対話を大切にしています。</p><blockquote style="margin:32px 0;padding:24px 0;border-top:1px solid var(--main);border-bottom:1px solid var(--line);font-size:1.4rem;font-weight:700;color:var(--main)">分からないまま決めさせない。選択肢と注意点を言葉にしてお伝えします。</blockquote><p class="message-sign">代表取締役　内田 豊</p></div></div></section>
...
ABOUT = page_hero("About", "会社情報", "不動産は、まず“知る”こと。", "wp-company.jpg", "現行サイトに掲載されている株式会社籠やの相談スペースと店舗外観") + f'''
...
INSIGHTS = page_hero("Insights", "知る・読みもの", "不動産の判断を急ぐ前に、確認したいこと。", "wp-finance.jpg", "資料と計算機を使って不動産の条件を整理する様子") + '''
...
PROPERTIES = page_hero("Properties", "物件情報", "募集状況や条件は、個別にご確認ください。", "wp-income.jpg", "現行サイトに掲載されている収益不動産の外観") + '''
<section class="page-sec"><div class="wrap page-intro"><div class="page-intro__pull">物件</div><div class="page-intro__body"><span class="eyebrow">Current information</span><h2>公開情報だけで判断せず、最新状況をご確認ください。</h2><p>販売・賃貸・管理物件の募集状況、価格、条件は変動します。ご希望のエリア、用途、予算を伺い、公開可否を含めて個別にご案内します。</p><div class="hero-actions"><a class="btn" href="contact.html">物件について問い合わせる</a><a class="btn btn-outline" href="purchase-asset.html">購入相談を見る</a></div></div></div></section>
<section class="page-sec alt"><div class="wrap"><div class="content-grid"><article class="info-card"><h3>居住用不動産</h3><p>マンション、戸建てなど。希望条件だけでなく、建物・管理・将来の売却も確認します。</p></article><article class="info-card"><h3>収益不動産</h3><p>一棟・区分の収益性、修繕、空室、融資、出口を整理します。</p></article><article class="info-card"><h3>賃貸・管理</h3><p>募集条件と管理体制を確認し、入居中の負担まで見通します。</p></article><article class="info-card"><h3>未公開情報</h3><p>物件情報の公開可否と条件は個別にご案内します。</p></article></div><p class="small-note">※写真は現行の株式会社籠や公式サイトに掲載されている物件・サービス画像です。募集状況は個別にご確認ください。</p></div></section>'''


CONTACT = page_hero("Contact", "お問い合わせ", "売ると決める前の段階から、分かる範囲でお聞かせください。", "wp-consultation.jpg", "不動産について相談する家族と担当者") + f'''
...
PRIVACY = page_hero("Privacy", "プライバシーポリシー", "お預かりする情報を適切に取り扱います。", "wp-company.jpg", "現行サイトに掲載されている株式会社籠やの相談スペースと店舗外観") + f'''
...
FEATURES = page_hero("Our approach", "籠やの考え方", "不動産は、まず“知る”こと。", "wp-invest.jpg", "住宅模型と資料を使って資産計画を検討するイメージ") + '''
...
FAQ_PAGE = page_hero("FAQ", "よくある質問", "売る前の相談で気になることへお答えします。", "wp-finance.jpg", "資料と計算機を使って不動産の条件を整理する様子") + f'''<section class="page-sec"><div class="wrap faq-layout"><div class="faq-sticky"><span class="eyebrow">FAQ</span><h2>相談前の確認事項</h2><p>掲載のない内容は、分かる範囲からお問い合わせください。</p></div><div class="faq-list">{faq_items()}</div></div></section>'''
...
PAGES = {
    ...
    "properties.html": ("物件情報｜株式会社籠や", "株式会社籠やが取り扱う物件情報へのご案内です。募集状況はお問い合わせ時にご確認ください。", PROPERTIES, ""),
