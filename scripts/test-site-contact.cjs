...
for (const [page, [heading, type]] of Object.entries(expectedPages)) {
  ...
  const expectedVersion = '20260916-readable';
...
assert.match(contactPage, /assets\/css\/contact-page\.css/);
assert.match(contactPage, /class="sample07-subpage contact-page"/);
assert.ok(channels[2][1].includes('contact-line__qr-card'), 'QR has its own frame');
assert.ok(channels[2][1].includes('LINEで友だち追加（新しいタブ）'), 'External LINE action has accessible label');
assert.match(contactPage, /class="wrap contact-grid contact-grid--stacked"/);
const refinements = fs.readFileSync(path.join(root, 'assets/css/site-refinements.css'), 'utf8');
assert.match(refinements, /\.contact-grid\.contact-grid--stacked\{grid-template-columns:minmax\(0,1fr\)/);
...
console.log('PASS: contact page stacks official form, phone, and LINE in source order');
