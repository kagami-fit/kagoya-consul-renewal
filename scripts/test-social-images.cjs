...
assert.equal((main.match(/<h1\b/g) || []).length, 1, 'One main heading');
assert.match(main, /<h1 id="tokyo-education-title"><span>小中学生起業家<\/span><span>教育プログラム<\/span><\/h1>/);
assert.match(main, /運営：東京都産業労働局/);
assert.match(main, /href="https:\/\/kigyouka-kyouiku\.metro\.tokyo\.lg\.jp\/"/);
assert.ok(main.indexOf('id="tokyo-education"') < main.indexOf('id="nbc-junior"'), 'Tokyo program leads the page');
assert.doesNotMatch(main, /class="nbc-activity-data"|<dt>(参加|対象|テーマ)<\/dt>|20名|防災グッズ/);
