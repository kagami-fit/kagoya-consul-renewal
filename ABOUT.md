...
株式会社籠やの会社案内PDFのカラー・トーンと確定HP設計書をもとに、不動産を「売る前に知る」ための相談サイトへ作り替えたプロジェクトです。写真は既存WordPress公式サイトの公開画像を使用しています。
...
- `src/wp-*`：現行WordPress公式サイトから取得・軽量化したサービス、相談、物件、事例素材
- `src/wp-image-sources.md`：各画像の取得元URLと用途の対応表
- `index.html`：トップページ
...
- PDF由来画像の不使用・WordPress公式画像への差し替え：完了
- トップページ：稼働中
- 主要下層ページ：稼働中
- ブログ3記事：稼働中
- GitHub Pages：稼働中
