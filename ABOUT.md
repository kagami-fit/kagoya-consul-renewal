...
- `src/gen-nbc-*`：社会貢献ページ専用に生成した、学び・メンターとの対話・教育活動の連携のイメージ4枚。各画像は一度だけ使用し、活動レポートの実写とは区別して表示
...
- `scripts/test-social-images.cjs`：`node scripts/test-social-images.cjs`で社会貢献ページ内の画像重複、実写の配置、生成画像の注記・軽量化、全フッターの `資産形成ラウンジ「エフクリ」` 表記を検証
...
- 社会貢献ページ：画像の繰り返しを解消。実際の授業写真2枚は活動レポートへ集約し、冒頭・学び・メンターとの対話・お問い合わせにはそれぞれ異なる生成イメージ4枚を配置。AI生成の注記付き。JET-ALL以降は引き続き非掲載
- フッター：事業名を `資産形成ラウンジ「エフクリ」` に統一し、共通生成元にも反映済み
- 相談ボタン：①「上品な輪郭線」を共通ヘッダーに採用。ロゴ左端固定と販売・成約物件の並びを維持
- お問い合わせセクション：PC⑤／モバイル③を正式採用。トップ・B案・主要下層ページの末尾に電話・フォーム・公式LINE付きで追加済み。5案の比較ページは参照用として維持
