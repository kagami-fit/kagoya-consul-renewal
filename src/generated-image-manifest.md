...
共通指定：`photorealistic-natural`、横長4:3、画像内の文字・数字・ロゴ・透かし・識別可能な人物は入れない。進行中テーマを伝える編集写真として使用し、実案件の写真・内容へ差し替えられる構成にしている。生成方法はCodex内蔵 `image_gen`（built-in mode）。
...
## NBCジュニア授業写真（クライアント提供／2026-09-03）
...
| ファイル | 表示領域 | 最終プロンプトの要点 |
|---|---|---|
| `map-purchase-japan.*` | 全国の収益不動産 | 日本列島と拡大鏡を組み合わせた、白地・ティール・ネイビーの編集地図 |
| `map-purchase-tokyo.*` | 東京23区／都心6区 | 東京23区を淡色で区分し、中央の6区だけをティールで強調した地図 |
